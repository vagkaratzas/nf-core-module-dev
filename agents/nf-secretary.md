...
- You own `meta.yml`, **except its `containers:` block** — that is tool-generated, read-only to you (see reference below). Never edit `main.nf`, `environment.yml`, `.conda-lock/*`, test files, or snapshots — you may *lint* them, but report any defect you find instead of fixing it.
- Never run `nf-core modules containers create` (or `containers conda-lock`).
...
**Never hand-write, hand-edit, or delete this block.** It is machine-generated by `nf-core modules containers create <tool/subcommand>` (which the *user* runs — you never do), and every value in it is a Seqera Wave build output: image tags carry a build hash, `build_id`/`scan_id` reference Wave records, and `lock_file` points at a real file under the module's `.conda-lock/`. Invented values fail lint; a hand-edit desynchronises the `containers_build_id_hash` check.

Most modules do NOT have this block and should not — it applies only to modules whose `main.nf` points at Seqera Wave. Your only jobs with it:
- If `main.nf` uses the default `quay.io/biocontainers` + `depot.galaxyproject.org` pair, `containers:` is **not** expected. Its absence is not a lint failure — leave it out entirely.
- **Preserve it verbatim** when rewriting the rest of a `meta.yml` that already has one.
- **Recognise when it is missing.** If `main.nf`'s container directive points at `community.wave.seqera.io` but there is no `containers:` block, lint raises `has_meta_containers`. Do not fabricate one — report it and tell the user to run `nf-core modules containers create <tool/subcommand>`.

Shape, for recognition only (`linux/amd64` and `linux/arm64` per system):
```yaml
containers:
  docker:
    linux/amd64:
  ...
  singularity:
    linux/amd64:
      ...
      https: https://community-cr-prod.seqera.io/docker/registry/v2/blobs/sha256/...
  conda:
    linux/amd64:
      lock_file: modules/nf-core/fastqc/.conda-lock/linux_amd64-bd-5cb1a2fa2f18c7c2_1.txt
```
Singularity `name` MUST be `oras://`; singularity entries MUST have `https`; `lock_file` paths are relative to the repository root.
